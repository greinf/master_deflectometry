    ...
    bool flipUnwrap_y_coordinates) 
{
    ...
    CV_Assert(stepwidth_x > 0 && stepwidth_y > 0);
    ...
    for (int row = 0; row < sz.height; row += stepwidth_y) {
        ...
        for (int cols = 0; cols < sz.width; cols += stepwidth_x) {
            ...
            // --- convert phase difference -> relativ to origin of display coord (left upper corner) ---
            const double X_pixel_rel = (phaseValX / CV_2PI) * wavelength;
            const double Y_pixel_rel = (phaseValY / CV_2PI) * wavelength;

            // --- pixels -> mm ---
            const double X_mm_rel = X_pixel_rel * pixe_pitch_mm;
            const double Y_mm_rel = Y_pixel_rel * pixe_pitch_mm;

            minX = std::min(minX, X_mm_rel);
    ...
    int n_shifts)
{
    ...
    for (int p = 0; p < 2; ++p) {
        ...
        wrapped[p] = cv::Mat(size, CV_64F, cv::Scalar(std::numeric_limits<double>::quiet_NaN()));
        contrast[p] = cv::Mat(size, CV_64F, cv::Scalar(std::numeric_limits<double>::quiet_NaN()));
    ...
    int row)
{
    ...
    int pixel_x = wrapped_reference.cols;

    double n_periods = static_cast<double>(pixel_x) / wavelength;

    const uchar* mask_ptr = mask.ptr<uchar>(row);
    const double* wrapped_ptr = wrapped.ptr<double>(row);
    const double* wrapped_ref_ptr = wrapped_reference.ptr<double>(row);
...
void ImageProcessing::unwrap_row(const cv::Mat& wrapped64, cv::Mat& unwrapped, const cv::Mat& mask, int row)
{
    ...
    int col)
{
    ...
    int k = 0;
    int pixel_y = wrapped_reference.rows;
    ...
    for (int row = 0; row < wrapped.rows; ++row)
    {
        ...
        const double wrapped_val = wrapped.ptr<double>(row)[col];
        const double wrapped_ref = wrapped_reference.ptr<double>(row)[col];

        k = static_cast<int>(std::round((wrapped_ref - wrapped_val) / CV_2PI));

        unwrapped.ptr<double>(row)[col] = wrapped_val + k * CV_2PI;
    }
}
    ...
    const double wavelength)
{
    ...
    // It is expected that all type checks are done before uwnrap_row(), or unwrap_column() is called
    // Necessary to speed up computation
    
    minmaxloc data = get_minmaxloc(wrapped[1]);
    CV_Assert(data.minval >= 0);
           
    cv::parallel_for_(cv::Range(0, wrapped[0].rows),
        [&](const cv::Range& range) {
            for (int row = range.start; row < range.end; ++row) {
                unwrap_row(wrapped[0], wrapped[1], unwrapped_phase[0], wavelength, mask, row);
            }
        }
    );
 
    data = get_minmaxloc(wrapped[3]);
    CV_Assert(data.minval >= 0);

    cv::parallel_for_(cv::Range(0, wrapped[2].cols),
        [&](const cv::Range& range) {
            for (int col = range.start; col < range.end; ++col) {
                unwrap_column(wrapped[2], wrapped[3], unwrapped_phase[1], wavelength, mask, col);
            }
        }
    );
    ...
    const cv::Mat& mask)
{
    ...
    CV_Assert(mask.type() == CV_8U);
    CV_Assert(wrapped[0].type() == CV_64FC1 &&
        wrapped[1].type() == CV_64FC1);

    std::vector<cv::Mat> unwrapped_phase(2);
    for (auto& m : unwrapped_phase) { 
        m = cv::Mat(
            wrapped[0].size(), 
            CV_64F, 
            cv::Scalar(std::numeric_limits<double>::quiet_NaN())); }
    ...
    for (std::size_t count = 0; count < wrapped.size(); count++) {
        ...
        {
            // vertical unwrap
            cv::parallel_for_(cv::Range(0, wrapped[0].cols),
                [&](const cv::Range& range) {
                    for (int col = range.start; col < range.end; ++col) {
