...
bool VimbaBackend::adjustSettings(const std::size_t camera_index) {
    ...
    std::vector<std::string> key{"Acquisition", "Exposure", "Trigger"};
    ...
    bool exposure = setExposureAbsRobust(cam, 3000.0);
    ...
    int width)
{
    ...
    if (!setIntClamped(cam, std::vector<std::string>{"Height"}, 2056)) return false;
    if (!setIntClamped(cam, std::vector<std::string>{"Width"}, 2464)) return false;
...
bool VimbaBackend::runAcquisition(const VmbCPP::CameraPtr& cam) {
    ...
    for (const auto& frames : m_frame_ptr) {
        ...
        if (frames->RegisterObserver(VmbCPP::IFrameObserverPtr(new FrameObserver(cam, m_ringBufferPtr))) != VmbErrorSuccess) {
...
void VimbaBackend::close(std::size_t i) 
{
    ...
    VmbCPP::FeaturePtr ft;
    if (m_running) {
        cam->GetFeatureByName("AcquisitionStop", ft);
        ft->RunCommand();
        cam->EndCapture();
        cam->FlushQueue();
        cam->RevokeAllFrames();
        for (const auto& frames : m_frame_ptr) {
            frames->UnregisterObserver();
        }
        m_running = false;
    }
...
std::vector<std::shared_ptr<defl::CameraConfig>> VimbaBackend::getCameraConfig() {
    return {};
}
