            ...
            std::numeric_limits<double>::infinity()) const noexcept
        {
            ...
            for (std::size_t i = 0; i < kDirectionCount; ++i)
            {
                const Eigen::Vector3d& normal{
                    s_boundingDirections[i]
                };

                const double projectedOrigin{
                    normal.dot(origin)
                };
                ...
                /*
                 * Der Strahl ist parallel zu den beiden Ebenen
                 * dieses Slabs.
                 */
                if (std::abs(projectedDirection) < epsilon)
                {
                    /*
                     * Liegt der Ursprung außerhalb des Slabs,
                     * kann der Strahl das Volumen nicht treffen.
                     */
                    if (projectedOrigin < min[i] ||
                        projectedOrigin > max[i])
                    {
                        return false;
                    }
                ...
                double t0{
                    (min[i] - projectedOrigin) / projectedDirection
                };

                double t1{
                    (max[i] - projectedOrigin) / projectedDirection
                };
...
public:
        ...
        double& vOut)
    {
        ...
        {
            const std::uint32_t nodeIndex{ nodeStack.back() };
            nodeStack.pop_back();
             ...
             */
            if (!node.bounds.intersect(origin, direction, tOut))
            {
                continue;
            }
