...
public:
    ...
    [[nodiscard]] bool intersectAny(
        const Eigen::Vector3d& origin,
        const Eigen::Vector3d& direction,
        const double maxDistance) const override
    {
        if (m_nodes.empty() || maxDistance <= 0.0)
        {
            return false;
        }
        ...
        while (stackSize > 0)
        {
            ...
            if (!node.bounds.intersect(projectedRay, maxDistance))
            {
                continue;
            }
            ...
            if (node.isLeaf())
            {
                ...
                for (std::size_t i = node.firstTriangle; i < end; ++i)
                {
                    const std::size_t triangleIndex{ m_triangleIndices[i] };
                    const Triangle& triangle{ m_triangle[triangleIndex] };
            ...
            {
                if (stackSize >= nodeStack.size())
                    throw std::runtime_error("BVH traversal stack capacity exceeded.");
                nodeStack[stackSize++] = node.leftChild;
            }
